...
CREATE DATABASE IF NOT EXISTS `quizmaster` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `quizmaster`;
...
CREATE TABLE notifications (
    ...
    type VARCHAR(50) NOT NULL, -- par ex., 'new_quiz', 'high_score', 'admin_message'
    message TEXT NOT NULL,
    related_id INT DEFAULT NULL, -- ID du quiz ou autre entité liée
...
CREATE TABLE `questions` (
  ...
  CONSTRAINT `questions_ibfk_1` FOREIGN KEY (`categorie_id`) REFERENCES `categories` (`id`) ON DELETE CASCADE,
  CONSTRAINT `questions_ibfk_2` FOREIGN KEY (`difficulte_id`) REFERENCES `difficultes` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `reponses_utilisateurs` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `quiz_complete_id` int(11) NOT NULL,
  `question_id` int(11) NOT NULL,
...
CREATE TABLE `badges` (
  ...
  `condition_score` int(11) DEFAULT NULL,
  `condition_nombre_quiz` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `categorie_id` (`categorie_id`),
  KEY `difficulte_id` (`difficulte_id`),
...
CREATE TABLE `quiz_progress` (
    ...
    `answers` JSON NOT NULL, -- Store answers as {question_id: option_id}
    `time_elapsed` INT NOT NULL DEFAULT 0, -- Total seconds elapsed
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (`utilisateur_id`) REFERENCES `utilisateurs` (`id`) ON DELETE CASCADE,
    FOREIGN KEY (`categorie_id`) REFERENCES `categories` (`id`) ON DELETE CASCADE,
    FOREIGN KEY (`difficulte_id`) REFERENCES `difficultes` (`id`) ON DELETE CASCADE,
    UNIQUE KEY `user_quiz_unique` (`utilisateur_id`, `categorie_id`, `difficulte_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `user_quiz_options` (
    ...
    `texte` TEXT NOT NULL,
    `est_correcte` TINYINT(1) NOT NULL DEFAULT 0,
    FOREIGN KEY (`user_quiz_question_id`) REFERENCES `user_quiz_questions` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE VIEW duel_leaderboard AS
SELECT 
    u.id,
    u.nom,
    u.est_contributeur,
    ...
    SUM(CASE WHEN d.winner_id IS NOT NULL AND d.winner_id != u.id AND d.winner_id != 0 THEN 1 ELSE 0 END) as losses,
    SUM(CASE WHEN d.winner_id = 0 THEN 1 ELSE 0 END) as draws,
    (SUM(CASE WHEN d.winner_id = u.id THEN 1 ELSE 0 END) / COUNT(d.id) * 100) as win_percentage,
    AVG(CASE 
        WHEN dr.total_questions > 0 
        THEN (dr.correct_answers / dr.total_questions) * 100 
        ELSE 0 
    END) as avg_accuracy,
    ...
    (SELECT type 
     ...
     GROUP BY type 
     ORDER BY COUNT(*) DESC 
     LIMIT 1) as preferred_duel_type
FROM utilisateurs u
LEFT JOIN duels d ON (d.challenger_id = u.id OR d.opponent_id = u.id) AND d.status = 'completed'
LEFT JOIN duel_results dr ON dr.user_id = u.id AND dr.duel_id = d.id
...
CREATE TABLE duel_leaderboard (
    ...
    losses DECIMAL(23,0),
    draws DECIMAL(23,0),
    win_percentage DECIMAL(30,4),
    avg_accuracy DECIMAL(21,8),
    avg_completion_time DECIMAL(14,4),
    last_duel_date DATETIME,
    preferred_duel_type VARCHAR(8),
    PRIMARY KEY (id)
);
);