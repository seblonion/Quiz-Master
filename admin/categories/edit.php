...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nom)) {
        ...
        // Tout est bon, mettre à jour la catégorie
        $result = mettreAJourCategorie($id, $nom, $description, $icone, $couleur);
        ...
        if ($result) {
            ...
            $erreur = 'Une erreur est survenue lors de la mise à jour de la catégorie';
        }
    }
}
    ...
    <div class="container">
        <section class="category-edit">
            <div class="section-header">
                <h1 class="section-title">Modifier la catégorie : <?= htmlspecialchars($categorie['nom']) ?></h1>
                <p class="section-description">Modifiez les détails de la catégorie ci-dessous</p>
            ...
            <form method="post" class="form-large">
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="hidden" id="icone" name="icone" value="<?= htmlspecialchars($categorie['icone']) ?>">
                        <div class="icon-preview">
                            <i id="icon-preview" class="fas <?= htmlspecialchars($categorie['icone']) ?>"></i>
                            <span id="icon-name"><?= $icones[$categorie['icone']] ?? 'Icône' ?></span>
                        </div>
                        <div class="icon-picker">
                            <?php foreach ($icones as $icone => $nom_icone): ?>
                                <div class="icon-option <?= ($icone === $categorie['icone']) ? 'selected' : '' ?>" data-icon="<?= $icone ?>" data-name="<?= $nom_icone ?>">
                                    <i class="fas <?= $icone ?>"></i>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="hidden" id="couleur" name="couleur" value="<?= htmlspecialchars($categorie['couleur']) ?>">
                        <div class="color-preview">
                            <div id="color-preview" style="background-color: <?= htmlspecialchars($categorie['couleur']) ?>;"></div>
                            <span id="color-name"><?= array_key_exists($categorie['couleur'], $couleurs) ? $couleurs[$categorie['couleur']] : 'Personnalisée' ?></span>
                        </div>
                        <div class="color-picker">
                            <?php foreach ($couleurs as $couleur => $nom_couleur): ?>
                                <div class="color-option <?= ($couleur === $categorie['couleur']) ? 'selected' : '' ?>" 
                        ...
                        <div class="custom-color">
                            ...
                            <input type="color" id="custom-color" class="form-control" value="<?= htmlspecialchars($categorie['couleur']) ?>">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="category-preview">
                        <div class="category-card" style="--category-color: <?= htmlspecialchars($categorie['couleur']) ?>;">
                            <div class="category-icon">
                                <i class="fas <?= htmlspecialchars($categorie['icone']) ?>"></i>
                            </div>
                            <div class="category-content">
                                <h3 class="category-title" id="preview-name"><?= htmlspecialchars($categorie['nom']) ?></h3>
                                <p class="category-description" id="preview-description"><?= htmlspecialchars($categorie['description']) ?></p>
                            </div>
                        </div>
                    </div>
                </div>
